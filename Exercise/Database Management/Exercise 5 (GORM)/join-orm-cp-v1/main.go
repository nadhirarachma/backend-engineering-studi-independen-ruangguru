...
func (t Teacher) Join(db *gorm.DB) ([]Joined, error) {
	...
	finalResult := []Joined{}
	result := db.Table("teachers").Select("teachers.name as teacher_name, schools.name as school_name, classes.name as class_name, lessons.name as lesson_name").Joins("left join schools on schools.id = teachers.school_id").Joins("left join classes on classes.id = teachers.class_id").Joins("left join lessons on lessons.id = teachers.lesson_id").Scan(&finalResult)
